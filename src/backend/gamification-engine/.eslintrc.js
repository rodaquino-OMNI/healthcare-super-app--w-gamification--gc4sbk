/**
 * ESLint configuration for the gamification-engine service
 * 
 * This configuration enforces code quality standards, consistent formatting,
 * and TypeScript best practices across the gamification-engine service.
...
module.exports = {
  ...
  rules: {
    ...
    'import/order': [
      'error',
      {
        ...
        'pathGroups': [
          ...
          // Internal gamification imports
          {
            pattern: '@app/gamification/**',
            group: 'internal',
            position: 'before',
          },
          {
    ...
    'no-restricted-imports': [
      'error',
      {
        patterns: [
          {
            group: ['../../../*'],
            message: 'Use path aliases (@app/*, @austa/*, @gamification/*) instead of relative paths',
          },
          {
            group: ['../../*'],
            message: 'Consider using path aliases (@app/*, @austa/*, @gamification/*) for deep imports',
          },
          {
            group: ['src/*'],
            message: 'Use path aliases (@app/gamification/* or @gamification/*) instead of src/* imports',
          },
          {
            group: ['../auth-service/*', '../health-service/*', '../care-service/*', '../plan-service/*'],
            message: 'Use path aliases (@app/auth/*, @app/health/*, @app/care/*, @app/plan/*) for cross-service imports',
          },
        ],
      },
    ],
    ...
    // Gamification-specific rules - enforces patterns specific to the gamification engine
    ...
    // Enforce proper achievement and rule handling
    'no-restricted-syntax': [
      'error',
      {
      ...
      {
        selector: 'MethodDefinition[key.name="unlockAchievement"][value.params.length<2]',
        message: 'Achievement unlocking should specify both user and achievement IDs',
      },
      {

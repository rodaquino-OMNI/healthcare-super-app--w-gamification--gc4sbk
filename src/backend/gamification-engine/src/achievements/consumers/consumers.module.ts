 ...
 * - Providing error handling and monitoring for all consumers
 */
@Module({
  imports: [
    ...
    KafkaModule.registerAsync({
      ...
      useFactory: (configService: ConfigService): KafkaConsumerOptions => ({
        ...
        consumer: {
          groupId: 'gamification-achievements-consumers',
  ...
  providers: [
    ...
    // Health journey consumer processes events from the Health service
    HealthJourneyConsumer,
    // Care journey consumer processes events from the Care service
    CareJourneyConsumer,
    // Plan journey consumer processes events from the Plan service
    PlanJourneyConsumer,
    // Provide factory for consumer configuration
    {
      ...
      useFactory: (configService: ConfigService) => ({
        ...
        dlq: {
          ...
          topicPrefix: 'dlq-achievements-',
          maxRetries: configService.get<number>('KAFKA_DLQ_MAX_RETRIES', 3),
        },
        // Configure topic subscriptions for each journey
        topics: {
          health: configService.get<string[]>('KAFKA_HEALTH_TOPICS', [
            'health-metrics',
            'health-goals',
            'health-insights',
            'health-devices',
          ]),
          care: configService.get<string[]>('KAFKA_CARE_TOPICS', [
            'care-appointments',
            'care-medications',
            'care-telemedicine',
            'care-plans',
          ]),
          plan: configService.get<string[]>('KAFKA_PLAN_TOPICS', [
            'plan-claims',
            'plan-benefits',
            'plan-selection',
            'plan-rewards',
...
export class ConsumersModule implements OnModuleInit {
  /**
   * Constructor logs initialization of the consumers module
  ...
  constructor(
    ...
    const healthTopics = this.configService.get<string[]>('KAFKA_HEALTH_TOPICS', [
      'health-metrics',
      'health-goals',
      'health-insights',
      'health-devices',
    ]);
    const careTopics = this.configService.get<string[]>('KAFKA_CARE_TOPICS', [
      'care-appointments',
      'care-medications',
      'care-telemedicine',
      'care-plans',
    ]);
    const planTopics = this.configService.get<string[]>('KAFKA_PLAN_TOPICS', [
      'plan-claims',
      'plan-benefits',
      'plan-selection',
      'plan-rewards',
    ...
    this.logger.log(
      'Initialized Achievement Consumers Module',
      {
        healthTopics,
        careTopics,
        planTopics,
        consumerGroup: this.configService.get<string>('KAFKA_CONSUMER_GROUP', 'gamification-achievements-consumers'),
  ...
  async onModuleInit(): Promise<void> {
    try {
      this.logger.log('Initializing achievement consumers...', {}, 'ConsumersModule');
      ...
      this.logger.log('All achievement consumers initialized successfully', {}, 'ConsumersModule');
    } catch (error) {
      this.logger.error(
        'Failed to initialize achievement consumers',
        {
          error: error.message,
          stack: error.stack,
        },
